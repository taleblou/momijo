...
# File: src/momijo/visual/echarts/line.mojo

from momijo.visual.echarts.core import json_string, json_number_array, json_string_array, json_array_str

fn echarts_line_option(x: List[String], y_series: List[List[Float64]], series_names: List[String],
                       smooth: Bool = True, area: Bool = False,
                       title: String = "Line Chart", y_name: String = "value") -> String:
    ...
    while i < len(y_series):
        ...
        var s = String("{type:\"line\",name:")+nm
        if smooth: s += String(",smooth:true")
        if area: s += String(",areaStyle:{}")
