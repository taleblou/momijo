...
# File: momijo/vision/backend/cpu/simd/convert_simd_u8_hwc.mojo
 
from vision.tensor import Tensor, packed_hwc_strides
from vision.dtypes import DType

fn rgb_to_gray_u8_hwc_simd(src: Tensor) -> Tensor:
    var h = src.height()
    var w = src.width()
    var c = src.channels()
    if c < 3:
