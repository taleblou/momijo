...
#  Description: Metal (MPS) kernel wrappers for GPU execution (with CPU fallback)
...
# NOTE:
# These are *wrappers* with CPU fallbacks so you can run smoke tests without Metal.
# Hook your actual Metal shaders/encoders where indicated.

# Elementwise add: C = alpha*A + beta*B
