...
#
# Reverse-mode autodiff engine (conceptual, dependency-light).
# Notes:
# - We deliberately avoid dictionaries/maps to reduce dependency surface.
# - Gradients are accumulated into a small list (GradEntry) using linear search.
# - This file assumes existence of Variable, Tape, GradRegistry, TensorHandle,
#   and a VJP runner (run_vjp). The rest of autograd modules can be completed
#   next; this engine is structured to integrate with them.
#
...
# -----------------------------
# Basic gradient entry
# -----------------------------
struct GradEntry:
    var var_id: Int
    var grad: TensorHandle

    fn __init__(out self, var_id: Int, grad: TensorHandle):
        self.var_id = var_id
        self.grad = grad
...
struct Engine:
    ...
    fn __init__(out self, registry: GradRegistry):
    ...
    fn backward(self, tape: Tape, y: Variable, seed_grad: TensorHandle):
...
fn Engine_accumulate_grad(mut x: Engine, grads: List[GradEntry], var_id: Int, g: TensorHandle):
    x.accumulate_grad(grads, var_id, g)

fn Engine_backward(x: Engine, tape: Tape, y: Variable, seed_grad: TensorHandle):
