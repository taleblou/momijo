...
#
# Module: momijo.nn.parameter
# Path:   src/momijo/nn/parameter.mojo
#
# Notes (Momijo style):
# - No `export`. No global variables. Use `var` (not `let`).
# - Constructors use `fn __init__(out self, ...)`. Prefer `mut/out` over `inout`.
# - Avoid exceptions; keep minimal dependencies.
#
# A light-weight, generic Parameter type for teaching and small tests.
# It stores a value and (optionally) a gradient of the same type, without assuming numeric traits.
...
struct Parameter[T: Copyable & Movable]:
    ...
    fn __init__(out self, data: T, requires_grad: Bool = True):
        self._data = data
    ...
    fn set_requires_grad(mut self, b: Bool):
    ...
    fn assign_(mut self, value: T, track_grad: Bool = True):
        self._data = value
