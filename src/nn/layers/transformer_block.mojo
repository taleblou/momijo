...
#
# Module: momijo.nn.transformer_block
# Path:   src/momijo/nn/transformer_block.mojo
#
# Minimal Transformer encoder block for pedagogy/smoke tests.
# Self-contained (no external imports): implements
#  - LayerNorm (over last dim)
#  - Multi-Head Self-Attention (list-based)
#  - Position-wise FFN (Linear -> activation -> Linear)
#  - Residual connections + (deterministic) inverted Dropout
#
# Shapes: sequence as List[List[Float64]] with shape [L, d_model].
#
# Momijo style:
...
struct LayerNorm:
    ...
    fn __init__(out self, D: Int, eps: Float64 = 1e-5):
        self.D = D
        self.eps = eps
        self.gamma = ones1d(D)
        self.beta = zeros1d(D)
...
struct MultiHeadAttention:
    ...
    fn __init__(out self, d_model: Int, num_heads: Int):
...
struct FeedForward:
    ...
    fn __init__(out self, d_model: Int, d_ff: Int):
...
struct TransformerBlock:
    ...
    fn __init__(out self, d_model: Int, n_heads: Int, d_ff: Int, dropout_p: Float64 = 0.1, training: Bool = True, seed: Int = 123):
        self.d_model = d_model
