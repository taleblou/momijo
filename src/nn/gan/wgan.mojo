...
#
# Module: momijo.nn.gan.wgan
# Path:   src/momijo/nn/gan/wgan.mojo
#
# Wasserstein GAN (WGAN) — compact, dependency-light scaffold.
# Uses Lists + Float64 with a simple critic (no sigmoid) and optional gradient
# penalty via a finite-difference proxy (to avoid heavy autograd deps).
#
# Momijo style:
...
fn tanh_like(x: Float64) -> Float64:
    # 2*sigmoid(2x)-1 via truncated exp series
    var e = _exp(-2.0 * x)
...
fn unit_dir(n: Int) -> List[Float64]:
    # deterministic +/- pattern, normalized ~ 1/sqrt(n)
    var u = List[Float64]()
    var s = 1.0
    for i in range(n):
        u.push(s); s = -s
...
fn grad_norm_proxy_1d(f_plus: Float64, f_minus: Float64, eps: Float64) -> Float64:
...
struct Critic:
    ...
    fn forward(self, x: List[Float64]) -> Float64:
        ...
        return self.l2.forward(h)[0]   # no sigmoid

# --- WGAN wrapper with optional GP ---
struct WGAN:
    var G: Generator
    var C: Critic
    ...
    fn __init__(out self, z_dim: Int, x_dim: Int, g_hidden: Int, c_hidden: Int, lambda_gp: Float64 = 10.0, eps_fd: Float64 = 1e-3):
    ...
    #  - Critic loss to MINIMIZE: (fake - real) + lambda*GP
    #  - Generator loss to MINIMIZE: -fake
    fn gen_step(self, z: List[Float64]) -> Float64:
        var x_fake = self.G.forward(z)
        var c_fake = self.C.forward(x_fake)
        return -c_fake

    fn _gp(self, x_real: List[Float64], x_fake: List[Float64]) -> Float64:
        if self.lambda_gp <= 0.0:
            return 0.0
        var x_hat = mix1d(x_real, x_fake, 0.5)
        ...
        var g = grad_norm_proxy_1d(f_plus, f_minus, self.eps_fd)
        var diff = g - 1.0
    ...
    fn dis_step(self, x_real: List[Float64], z: List[Float64]) -> (Float64, Float64, Float64):
        ...
        var x_fake = self.G.forward(z)
        var c_fake = self.C.forward(x_fake)
        var gp = self._gp(x_real, x_fake)
        var loss_c = (c_fake - c_real) + gp
        return (loss_c, c_real, c_fake)
...
fn _self_test() -> Bool:
    ...
    var m = WGAN(8, 12, 16, 16, 5.0, 1e-3)

    var g_loss = m.gen_step(z)
    ok = ok and (g_loss == g_loss)  # NaN guard

    var (d_loss, c_r, c_f) = m.dis_step(x_real, z)
    ok = ok and (d_loss == d_loss) and (c_r == c_r) and (c_f == c_f)

    # Repeat a few times
    for t in range(3):
        var (dd, rr, ff) = m.dis_step(x_real, z)
        var gg = m.gen_step(z)
        ok = ok and (dd == dd) and (gg == gg)
