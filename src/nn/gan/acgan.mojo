...
#
# Module: momijo.nn.gan.acgan
# Path:   src/momijo/nn/gan/acgan.mojo
#
# A compact, dependency-light ACGAN scaffold for pedagogy and smoke tests.
# Uses Lists and Float64 math (no tensor dependency) to keep build simple.
# Replace internals with real tensor ops when wiring to momijo.tensor.*.
#
# Design notes (Momijo style):
# - No global vars, no `export`. Use `var` (no `let`).
# - Constructors: `fn __init__(out self, ...)`.
# - Prefer `mut/out` over `inout`.

# --- Tiny helpers (repeat minimal code instead of importing heavy deps) ---
...
struct Generator:
    ...
    fn __init__(out self, z_dim: Int, num_classes: Int, hidden_dim: Int, out_dim: Int):
        self.z_dim = z_dim
...
struct DiscriminatorAux:
    ...
    fn __init__(out self, in_dim: Int, hidden_dim: Int, num_classes: Int):
        self.in_dim = in_dim
        self.num_classes = num_classes
...
struct ACGAN:
    ...
    fn __init__(out self, z_dim: Int, num_classes: Int, g_hidden: Int, d_hidden: Int, x_dim: Int, lambda_cls: Float64 = 1.0):
