...
#
# Module: momijo.nn.lr_scheduler
# Path:   src/momijo/nn/lr_scheduler.mojo
#
# Lightweight learning-rate schedulers (self-contained).
# These structs track `last_epoch` and `current_lr`, and expose:
#   - step()         -> advance one epoch/step and update lr
#   - set_epoch(k)   -> jump to an absolute epoch
#   - get_lr()       -> read current lr
#
# Schedulers:
#   - StepLR(base_lr, step_size, gamma)
#   - MultiStepLR(base_lr, milestones[Int...], gamma)
#   - ExponentialLR(base_lr, gamma)
#   - CosineAnnealingLR(base_lr, T_max, eta_min)
#   - CosineAnnealingWarmRestarts(base_lr, T_0, T_mult, eta_min)
#   - LinearWarmup(base_lr, warmup_steps, start_lr)
#   - WarmupCosine(base_lr, total_steps, warmup_steps, eta_min, start_lr)
#
# Notes:
# - No parameter groups. You can manually apply the returned `current_lr`
#   to your optimizer (e.g., AdamW.set_lr(current_lr)).
# - Epoch semantics: at construction, last_epoch=0 and current_lr=base_lr.
#   Calling step() increments epoch (-> 1) and recomputes lr for that epoch.
...
struct ExponentialLR:
    ...
    fn __init__(out self, base_lr: Float64, gamma: Float64 = 0.99):
        self.base_lr = base_lr
        self.gamma = gamma
        self.last_epoch = 0
        self.current_lr = base_lr
...
struct CosineAnnealingWarmRestarts:
    ...
    fn __init__(out self, base_lr: Float64, T_0: Int, T_mult: Int = 2, eta_min: Float64 = 0.0):
        if T_0 <= 0: T_0 = 1
...
struct LinearWarmup:
    ...
    fn __init__(out self, base_lr: Float64, warmup_steps: Int, start_lr: Float64 = 0.0):
...
struct WarmupCosine:
    ...
    fn __init__(out self, base_lr: Float64, total_steps: Int, warmup_steps: Int = 0, eta_min: Float64 = 0.0, start_lr: Float64 = 0.0):
...
struct _MockOpt:
    var lr: Float64
    fn __init__(out self, lr: Float64): self.lr = lr
