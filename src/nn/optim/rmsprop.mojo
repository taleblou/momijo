...
#
# Module: momijo.nn.rmsprop
# Path:   src/momijo/nn/rmsprop.mojo
#
# RMSprop optimizer (dependency-light, list-based).
#
# Features:
#   - Supports Param1D / Param2D
#   - Square-avg accumulator with decay `alpha`
#   - Optional momentum buffer (PyTorch-like)
#   - Optional centered variant: uses moving avg of grad as well
#   - L2 weight decay (i.e., grad += wd * param)
#   - Utilities: add_param, zero_grad, set_lr, set_weight_decay, reset_state
#
# Update rules (per-parameter):
#   s_t = alpha * s_{t-1} + (1 - alpha) * g_t^2
#   if centered: m_t = alpha*m_{t-1} + (1-alpha)*g_t; denom = sqrt(s_t - m_t^2 + eps)
#   else:        denom = sqrt(s_t + eps)
#   if momentum > 0:
#       buf = momentum*buf + g_t / denom
#       p   = p - lr * buf
#   else:
#       p   = p - lr * g_t / denom
#
# Notes:
# - This is a toy/educational implementation to keep the codebase self-contained.
...
struct RMSprop:
    ...
    fn __init__(out self, lr: Float64 = 1e-3, alpha: Float64 = 0.99, eps: Float64 = 1e-8, weight_decay: Float64 = 0.0, momentum: Float64 = 0.0, centered: Bool = False):
        self.lr = lr
        self.alpha = alpha
        self.eps = eps
    ...
    fn set_weight_decay(mut self, wd: Float64): self.weight_decay = wd
