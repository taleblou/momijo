...
#
# Module: momijo.nn.bce_with_logits
# Path:   src/momijo/nn/bce_with_logits.mojo
#
# Binary Cross Entropy with Logits (BCEWithLogits) — dependency-light.
# Stable formulation using softplus:
#   loss(x, y) = softplus(x) - x*y
# With positive class weighting (PyTorch-compatible):
#   loss_pw(x, y; pos_w) = (1 - y)*softplus(x) + pos_w*y*softplus(-x)
# Optional per-element weights and reduction: "mean" | "sum" | "none".
#
# Shapes supported:
#   - 1D: logits[List[Float64]], targets[List[Float64]]
#   - 2D: logits[List[List[Float64]]], targets[List[List[Float64]]]
#
# Momijo style:
