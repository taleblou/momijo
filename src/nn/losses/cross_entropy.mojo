...
#
# Module: momijo.nn.cross_entropy
# Path:   src/momijo/nn/cross_entropy.mojo
#
# Cross Entropy loss for classification with logits — dependency-light.
# Stable log-softmax based formulation with:
#   - reduction: "mean" | "sum" | "none"
#   - class weights (per-class)
#   - ignore_index (exclude items from mean/sum denominator)
#   - label_smoothing epsilon in [0,1)
#
# Shapes supported:
#   - 1D: logits[List[Float64]] and target index (Int)
#   - 2D: logits[List[List[Float64]]] with targets[List[Int]] (class indices)
#   - Optional: one-hot targets via cross_entropy2d_onehot
#
# Momijo style:
