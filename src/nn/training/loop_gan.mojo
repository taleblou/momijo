...
#
# Module: momijo.nn.loop_gan
# Path:   src/momijo/nn/loop_gan.mojo
#
# A tiny, dependency-light GAN training loop on a 1D toy distribution.
# - Generator G(z) = a*z + b
# - Discriminator D(x) = sigmoid(w*x + c)
# - Losses: 
#     * Discriminator (maximization, implemented as minimizing negative): 
#           L_D = -E[log D(x_real)] - E[log (1 - D(x_fake))]
#     * Generator (non-saturating):
#           L_G = -E[log D(x_fake)]
# - Optimizer: simple SGD (separate lr for G and D)
#
# Dataset (deterministic, no RNG):
#   Real distribution is uniform on [1.5, 2.5] with mean=2.0.
#   Latent z is uniform on [-1, 1].
#   The optimum for G is a=0.5, b=2.0 so that G(z) ~ U[1.5,2.5].
#
# Exposes:
#   struct GanParamsG { a, b }
#   struct GanParamsD { w, c }
#   struct GanLog { d_losses[], g_losses[], mu_real[], mu_fake[] }
#   fn train_gan(epochs, batch_size, lr_d, lr_g) -> (GanParamsG, GanParamsD, GanLog)
#
# Self-test trains for a short run and checks G close to (0.5, 2.0).
...
struct GanParamsG:
    var a: Float64
    var b: Float64
    fn __init__(out self, a: Float64 = 0.1, b: Float64 = 0.0):
        self.a = a
        self.b = b
...
struct GanParamsD:
    var w: Float64
    var c: Float64
    fn __init__(out self, w: Float64 = 0.0, c: Float64 = 0.0):
        self.w = w
        self.c = c
