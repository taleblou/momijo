...
#
# Module: momijo.nn.visualize
# Path:   src/momijo/nn/visualize.mojo
#
# Lightweight visualization helpers without external dependencies.
# Produces ASCII charts/plots and ASCII PPM images (as strings).
#
# Included:
#   - sparkline(values)                              -> String
#   - line_chart_ascii(values, height=10, fill=False)-> String
#   - hist_ascii(values, bins=10, height=10)         -> String
#   - cm_ascii_heatmap(cm[Int][Int])                 -> String
#   - ppm_from_gray(img[H][W] float in [0,1])        -> String (P3)
#   - ppm_from_heatmap(mat[H][W] float, vmin,vmax)   -> String (P3, blue->red)
#
# Notes:
# - We avoid file I/O. Callers can write returned strings to files if desired.
# - ASCII focus keeps this module portable (for smoke tests and logs).
