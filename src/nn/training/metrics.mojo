...
#
# Module: momijo.nn.metrics
# Path:   src/momijo/nn/metrics.mojo
#
# Lightweight metrics for classification & regression.
# Self-contained (List-based Float64/Int math), no external deps.
#
# Classification:
#   - accuracy_from_labels(preds[Int], targets[Int])
#   - accuracy_from_scores(scores[List[List[Float64]]], targets[Int])  # argmax
#   - topk_accuracy(scores[N][C], targets[N], k)
#   - confusion_matrix(targets[N], preds[N], num_classes=-1) -> Int[Nc][Nc]
#   - precision_recall_f1_from_cm(cm) -> per-class PRF
#   - micro_f1_from_cm(cm), macro_f1_from_cm(cm), weighted_f1_from_cm(cm)
#   - classification_report(cm) -> String
#   - binary_auroc(scores[N], targets[N in {0,1}])
#
# Regression:
#   - mae(y_true, y_pred), mse, rmse, r2_score
#
# Utilities:
#   - AverageMeter (running average)
#
# A small self-test at the bottom validates behavior on toy data.
...
struct AverageMeter:
    ...
    fn update(mut self, val: Float64, n: Int = 1):
