...
fn groupby(frame: DataFrame, by: List[String], aggs: Dict[String, List[String]]) -> DataFrame:
    var out = DataFrame()
    var n = frame.nrows()
    var group_map: Dict[String, List[Int]] = Dict[String, List[Int]]()
    var i = 0
    while i < n:
        var key_parts: List[String] = List[String]()
        for col in by:
            var v = frame.get_column(col).get_string(i)
            key_parts.append(v)
        var key = String("_").join(key_parts)
        var lst_opt = group_map.get(key)
        var lst: List[Int]
        if lst_opt is None:
            lst = List[Int]()
        else:
        ...
        group_map[key] = lst
        i += 1
    for entry in group_map.items():
        var key = entry.key
        var indices = entry.value
        var row: Dict[String, String] = Dict[String, String]()
        var key_values = key.split("_")
        var j = 0
        var n_by = len(by)
        while j < n_by:
            var gv = String("")
            if j < len(key_values):
                gv = String(key_values[j])
            row[by[j]] = gv
            j += 1
        for entry2 in aggs.items():
            var col = entry2.key
            var funcs = entry2.value
            var series = frame.get_column(col)
            var col_values: List[Float64] = List[Float64]()
            var k = 0
            var m = len(indices)
            while k < m:
                var idx = indices[k]
                var s = series.get_string(idx)
                var x = _parse_f64_safe(s)
                col_values.append(x)
                k += 1
            for func in funcs:
                var agg_val = _apply_agg_local_f64(col_values, func)
                row[String(col + "_" + func)] = String(agg_val)
        var rp = make_pairs()
        for e in row.items():
            var rk = e.key
            var rv = e.value
            ...
            single.append(rv)
            rp = pairs_append(rp, rk, single)
        var row_df = df_from_pairs(rp)
        var parts = List[DataFrame]()
        parts.append(out)
        parts.append(row_df)
        out = concat_rows(parts, True)
    return out
