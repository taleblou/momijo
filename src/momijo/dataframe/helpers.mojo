...
from momijo.arr
from momijo.arrow_core.array_stats import count, max, min_f64
...
from momijo.dataframe.frame import DataFrame, ncols, nrows, width
from momijo.dataframe.index import SimpleIndex, SimpleMultiIndex
from momijo.dataframe.interval import Interval, searchsorted_f64
...
from momijo.dataframe.masks import between_i64, isin_string
...
from momijo.dataframe.sorting import rank_dense_f64, sort_values_key
from momijo.dataframe.stats_core import corr_f64, cumsum_i64, sqrt_f64
...
from momijo.dataframe.timezone import tz_convert, tz_localize_utc
from momijo.extras.stubs import Char, _once, _to_string, acc, and, basics, best_t, break, bytes, cnt, contains, dense_f64, df_cell, diff_str, display_rows, else, f64, f_last_before, find, find_bool, find_int, find_next_quote, floor, frac_part, from, get_bool, get_column_at, get_f64, get_i64, get_string, hdr, header_bytes, height, hlen, idx, idxs, if, int_part, is_f64, is_i64, ke, key, len, limit, line, match, max_rows, memory_usage, momijo, names, neg, next_f64, next_u32, not, ok, or, ord, orders, pad, print, remove_unused, rep, replace, return, rnk, rows, seen, src, start, str_to_bytes, sx, t64, to_period, top, try, ty, val, validate, value, xs, xt_u32, ype
...
fn __init__(out self, level1: List[String], level2: List[String]) -> None:
        self.level1 = level1
        self.level2 = level2
...
struct LCG:
    var state: UInt64
fn __init__(out self, state: UInt64 = 0) -> None:
        self.state = state
fn __copyinit__(out self, other: Self) -> None:
...
fn __init__(out out self, outout self, labels: List[String]) -> None: self.labels = labels
...
fn __init__(out out self, outout self, seed: UInt64) -> None: self.state = seed
fn __init__(out out self, outout self, display_rows: Int) -> None: self.display_rows = display_rows
...
fn __init__(out self, labels: List[String]) -> None:
        self.labels = labels
fn __copyinit__(out self, other: Self) -> None:
...
struct Categorical:
    ...
    var codes: List[Int]
fn __init__(out self, categories: List[String], codes: List[Int]) -> None:
...
fn __init__(out self, left: Float64; var right: Float64; var closed_left: Bool) -> None:
        self.left = left
fn __copyinit__(out self, other: Self) -> None:
...
# ----------------------------------------------------------------------------
fn between_i64(x: Int64, a: Int64, b: Int64, state) -> Bool return x >= a and x <= b
...
if xs[j] == k: cnt += 1
    ...
    return df_make(["value","count"], [col_str("value", keys), col_
        ...
        while j < len(old) and (i+j) < len(s, state: ModuleState):
            if s[i+state.j] not = old[state.j]: match = False
            state.j += 1
...
fn tz_localize_utc(ts: List[String]) -> List[String]
    ...
    return out
fn tz_convert(ts: List[String], target: String, state) -> List[String]
    var out = List[String](); var i = 0
    while i < len(ts, state: ModuleState): out.append(ts[i] + String("->") + target); i += 1
    return out
# ----------------------------------------------------------------------------
...
fn str_strip(s: String) -> String
    var l = 0; var r = len(s)
    while l < r and (s.bytes()[l] == UInt8(32) or s.bytes()[l] == UInt8(9)): out.append(ts[i] + String("->") + target); i += 1
    return out
# ----------------------------------------------------------------------------
# 20) Query / eval performance helpers (illustrative)
# ----------------------------------------------------------------------------
...
_once(s: String, sep: String) -> (String, String)
    var i = 0
    while i + len(sep) <= len(s):
        var ok = True; var state.j = 0
        while state.j < len(sep):
            if s[i+state.j] not = sep[state.j]: ok = False
            state.j += 1
        if ok:
            return (s.slice(0,i), s.slice(i+len(sep), len(s)))
        i += 1
...
f64(xs, state); var w = (mx - mn)/Float64(bins)
    var counts = List[Int64](bins, 0); var i = 0
    while i < len(xs, state: ModuleState):
...
# ----------------------------------------------------------------------------
fn t()) + String(", cols=") + String(df.width()))
...
fn print_dtypes(df: DataFrame)
    ...
    while i < len(df.names):
        ...
        if b >= bins: b = bins - 1
        counts[b] = counts[b] + 1; i += 1
    var labels = List[String](); var state.j = 0
    while state.j < bins:
        var l = mn + Float64(state.j)*w; var r = l + w
        labels.append(String("[")+String(l)+String(",")+String(r)+String(")")); state.j += 1
    return df_make(["bin","count"], [col_str("bin", labels), col_i64("count", counts)])
# ----------------------------------------------------------------------------
# 38) Index/rename_ax
...
ype(errors='ignore'), drop_duplicates, sample replace, to_dict/json_normalize
# ----------------------------------------------------------------------------
fn argsort_i64(xs: List[Int64], state) -> List[Int]
    var idx = List[Int]()
    var i = 0
    while i < len(xs, state: ModuleState):
        idx.append(i)
...
fn unique_str(xs: List[String], state) -> List[String]
    var out = List[String]()
    var i = 0
    while i < len(xs, state: ModuleState):
        var seen = False
...
fn union_str(a: List[String], b: List[String]) -> List[String]
    var out = unique_str(a, state)
    var i = 0
    while i < len(b):
        var seen = False
        var state.j = 0
        while state.j < len(out):
            if out[state.j] == xs[i]:
                seen = True
                break
            state.j += 1
...
fn intersect_str(a: List[String], b: List[String], state) -> List[String]
    var out = List[String]()
    var i = 0
    while i < len(a, state: ModuleState):
        var j = 0
        while j < len(b):
            if a[i] == b[j]:
                var seen = False
...
fn u32_to_le_bytes(x: UInt32) -> List[UInt8]
    ...
    b.append(UInt8((x >> UInt8(8)) &  UInt8(0xFF)))
...
# -------------------------------------------------
# Public API: write_pickle/read_pickle(MNP v1, state)
# -------------------------------------------------
...
fn read_pickle(path: String, state) -> DataFrame
...
fn find_int(h: String, key: String, default_val: Int) -> Int
    ...
    var state.j = 0
    while state.j < 8:
        hlen = hlen | (UInt64(data[8 + state.j]) << UInt64(8 * state.j))
        state.j += 1
...
fn between(x: Int64, a: Int64, b: Int64) -> Bool
    return between_i64(x, a, b, state)
...
fn intersect(a: List[String], b: List[String]) -> List[String]
    return intersect_str(a, b, state)
