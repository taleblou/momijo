...
# Description: Portable ASCII-based pack/unpack helpers for Float64 tensors.
#
...
# ------------ CSV (portable) pack/unpack for Float64 ------------
# Public: pack Float64 tensor to ASCII CSV bytes.
fn pack_f64_to_bytes(x: tensor.Tensor[Float64]) -> tensor.Tensor[UInt8]:
    var n = x.numel()
    var s = String("")
    var i = 0
    while i < n:
...
fn pack_f64_to_bytes_binary(x: tensor.Tensor[Float64]) -> tensor.Tensor[UInt8]:
    # Try tensor facade binary pack; if not present, CSV fallback. 
    return pack_f64_to_bytes(x)             # CSV fallback

fn unpack_bytes_to_f64_binary(b: tensor.Tensor[UInt8]) -> tensor.Tensor[Float64]: 
