...
# File:         src/momijo/learn/engine/hooks.mojo
#
# Description:
#   Training event system (hooks) for Momijo Learn. Provides a lightweight,
#   backend-agnostic pub/sub mechanism to register handlers for training
#   lifecycle events (train/valid/test).
#
...
struct TrainState:
    ...
    fn __str__(self) -> String:
        return "TrainState(epoch=" + String(self.epoch) +
               ", step=" + String(self.global_step) +
               ", lr=" + String(self.lr) +
               ", train_loss=" + String(self.train_loss) +
               ", val_loss=" + String(self.val_loss) + ")"
...
struct BatchInfo:
    ...
    fn __str__(self) -> String:
        return "BatchInfo(index=" + String(self.batch_index) +
...
struct LRUpdateInfo:
    ...
    fn __str__(self) -> String:
        return "LRUpdateInfo(prev=" + String(self.prev_lr) +
               ", new=" + String(self.new_lr) + ")"
...
struct Hooks:
    ...
    fn add_on_train_start(mut self, h: fn(mut TrainState)) -> None:
        self._on_train_start.append(h)

    fn add_on_train_end(mut self, h: fn(mut TrainState)) -> None:
        self._on_train_end.append(h)

    fn add_on_epoch_start(mut self, h: fn(mut TrainState)) -> None:
        self._on_epoch_start.append(h)

    fn add_on_epoch_end(mut self, h: fn(mut TrainState)) -> None:
        self._on_epoch_end.append(h)
    ...
    fn add_on_validation_start(mut self, h: fn(mut TrainState)) -> None:
        self._on_validation_start.append(h)

    fn add_on_validation_end(mut self, h: fn(mut TrainState)) -> None:
        self._on_validation_end.append(h)

    fn add_on_test_start(mut self, h: fn(mut TrainState)) -> None:
        self._on_test_start.append(h)

    fn add_on_test_end(mut self, h: fn(mut TrainState)) -> None:
        self._on_test_end.append(h)

    fn add_on_lr_update(mut self, h: fn(mut TrainState, LRUpdateInfo)) -> None:
        self._on_lr_update.append(h)

    fn add_on_checkpoint_saved(mut self, h: fn(mut TrainState, String)) -> None:
        self._on_checkpoint_saved.append(h)
    ...
    fn on_train_start(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_train_start):
            self._on_train_start[i](state)
            i = i + 1

    fn on_train_end(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_train_end):
            self._on_train_end[i](state)
            i = i + 1

    fn on_epoch_start(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_epoch_start):
            self._on_epoch_start[i](state)
            i = i + 1

    fn on_epoch_end(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_epoch_end):
            self._on_epoch_end[i](state)
            i = i + 1

    fn on_batch_start(mut self, mut state: TrainState, info: BatchInfo) -> None:
        var i = 0
        while i < len(self._on_batch_start):
    ...
    fn on_validation_start(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_validation_start):
            self._on_validation_start[i](state)
            i = i + 1

    fn on_validation_end(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_validation_end):
            self._on_validation_end[i](state)
            i = i + 1

    fn on_test_start(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_test_start):
            self._on_test_start[i](state)
            i = i + 1

    fn on_test_end(mut self, mut state: TrainState) -> None:
        var i = 0
        while i < len(self._on_test_end):
            self._on_test_end[i](state)
            i = i + 1

    fn on_lr_update(mut self, mut state: TrainState, info: LRUpdateInfo) -> None:
        var i = 0
        while i < len(self._on_lr_update):
            self._on_lr_update[i](state, info)
            i = i + 1

    fn on_checkpoint_saved(mut self, mut state: TrainState, path: String) -> None:
        var i = 0
        while i < len(self._on_checkpoint_saved):
