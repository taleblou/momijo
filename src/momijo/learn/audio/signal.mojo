...
#   Float32-based, no external deps. Compatible with Momijo style.
...
fn _wrap_pi(x: Float32) -> Float32:
    ...
    var y = x - TAU * Float32(Int(x / TAU))
    if y >  PI:  y = y - TAU
    if y < -PI:  y = y + TAU
    return y

fn _cos(x: Float32) -> Float32:
    var xx = _wrap_pi(x)
    var z = xx * xx
    var c = 1.0
    c = c - (z * 0.5)
...
fn _sin(x: Float32) -> Float32:
    var PI_2 = 1.5707963267948966
    return _cos(x - PI_2)

fn hann_window(win_len: Int) -> tensor.Tensor[Float32]:
    ...
    var PI = 3.141592653589793
    while n < win_len:
        var a = 2.0 * PI * Float32(n) / Float32(win_len - 1)
        w._data[n] = 0.5 - 0.5 * _cos(a)
        n = n + 1
    return w.copy()

fn frame_signal(x: tensor.Tensor[Float32], frame_len: Int, hop: Int) -> List[tensor.Tensor[Float32]]:
    var L = x.shape()[0]
    var out = List[tensor.Tensor[Float32]]()
    if L < frame_len:
        return out.copy()
...
fn dft_power(frame: tensor.Tensor[Float32], n_fft: Int) -> tensor.Tensor[Float32]:
    var L = frame.shape()[0]
    var half = n_fft // 2
    ...
    while k <= half:
        ...
        var n = 0
        while n < L:
            var ang = 2.0 * PI * Float32(k * n) / Float32(n_fft)
            var cn = _cos(ang)
            var sn = _sin(ang)
