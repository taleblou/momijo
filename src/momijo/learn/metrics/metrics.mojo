...
fn _row_sum(x: tensor.Tensor[Float32]) -> tensor.Tensor[Float32]:
    var C = x.shape()[1]
    return tensor.matmul(x, tensor.ones([C,1]))

fn accuracy_approx(logits: tensor.Tensor[Float32], target_onehot: tensor.Tensor[Float32]) -> Float32:
    ...
    return total.item() / Float32(N)



# --- helpers ---------------------------------------------------------

fn _scalar(v: Float32) -> Tensor[Float32]:
    var data = List[Float32]()
    data.append(v)
    return Tensor[Float32](data, [1])

fn _reduce_mean(x: Tensor[Float32]) -> Tensor[Float32]:
    return x.mean(None, False)

fn _reduce_sum(x: Tensor[Float32]) -> Tensor[Float32]:
    return x.sum(None, False)

# --- classification metrics -----------------------------------------

# accuracy: if logits, we take argmax over 'axis'; if already labels, pass same axis length 1.
fn accuracy(pred: Tensor[Float32], target: Tensor[Float32], axis: Int = -1) -> Tensor[Float32]:
...
fn precision_recall_f1(pred: Tensor[Float32], target: Tensor[Float32], axis: Int = -1, eps: Float32 = 1e-12) -> Tensor[Float32]:
    var yhat = pred.argmax(Optional[Int](axis))
    var y = target
    ...
    var prf = List[Float32]()
    var c = 0
    while c < C:
        var cls = _scalar(Float32(c))
    ...
    return Tensor[Float32](out, [3])

# --- segmentation / set metrics -------------------------------------

# IoU (Jaccard) for binary masks. For multi-class, call per-class and average.
fn iou(pred_mask: Tensor[Float32], target_mask: Tensor[Float32], eps: Float32 = 1e-6) -> Tensor[Float32]:
    var p = pred_mask
    var t = target_mask
...
fn psnr(img: Tensor[Float32], ref: Tensor[Float32], max_val: Float32 = 1.0, eps: Float32 = 1e-12) -> Tensor[Float32]:
    ...
    var val = 10.0 * (Float32.log(ratio) / 2.302585092994046)
    return _scalar(val)
...
fn ssim(x: Tensor[Float32], y: Tensor[Float32], k1: Float32 = 0.01, k2: Float32 = 0.03, win: Int = 11) -> Tensor[Float32]:
    var L = 1.0
    var c1 = (k1 * L) * (k1 * L)
    var c2 = (k2 * L) * (k2 * L)
