# Project:      Momijo
# Module:       src.momijo.enum.enumset_dyn
# File:         enumset_dyn.mojo
# Path:         src/momijo/enum/enumset_dyn.mojo
#
# Description:  src.momijo.enum.enumset_dyn — focused Momijo functionality with a stable public API.
...
# Notes:
#   - Structs: DynEnumSet
#   - Key functions: popcount_u64, lower_mask, __init__, __init__, __len__, clear, in_domain, which ...
...
from momijo.enum.meta import EnumMetaInfo, all_tags
...
fn __init__(out self, k: Int) -> None:
        var d = k
        if d < 0:
            d = 0
        if d > 256:
...
# Inputs: mut s, tag.
# Returns: result value or status.
fn enumset_dyn_add(mut s: DynEnumSet, tag: Int) -> None:
...
# Inputs: mut s, tag.
# Returns: result value or status.
fn enumset_dyn_remove(mut s: DynEnumSet, tag: Int) -> None:
    if tag < 0 or tag >= s.k: return
    var idx = tag # 64
    var bit = UInt64(1) << UInt64(tag % 64)
    s.chunks[idx] = s.chunks[idx] & (~bit)
