# Project:      Momijo
# Module:       src.momijo.nn.optim.adamw
# File:         adamw.mojo
# Path:         src/momijo/nn/optim/adamw.mojo
#
...
#
# Notes:
#   - Structs: Param1D, Param2D, AdamW
#   - Key functions: zeros1d, zeros2d, copy1d, copy2d, _sqrt_pos, _pow, _abs, __init__ ...
...
fn __init__(out self, lr: Float64 = 1e-3, beta1: Float64 = 0.9, beta2: Float64 = 0.999, eps: Float64 = 1e-8, weight_decay: Float64 = 0.0) -> None:
        self.lr = lr
        self.beta1 = beta1
        self.beta2 = beta2
        self.eps = eps
...
fn set_lr(mut self, lr: Float64) -> None:
        self.lr = lr
fn set_weight_decay(mut self, wd: Float64) -> None:
...
fn _step_param1d(mut self, idx: Int) -> None:
        var p = self.p1[idx]
        var m = self.m1[idx]
        var v = self.v1[idx]
        var n = len(p.data)
        ...
        self.m1[idx] = m
        self.v1[idx] = v
fn _step_param2d(mut self, idx: Int) -> None:
        var p = self.p2[idx]
        var m = self.m2[idx]
        var v = self.v2[idx]
        var R = len(p.data)
