...
fn reset(mut self) -> None:
        ...
        assert(self is not None, String("self is None"))
        self.value() = 0.0
fn update_from_counts(mut self, correct_add: Int, total_add: Int) -> None:
        self.correct += correct_add
        self.total += total_add
        if self.total > 0:
            assert(self is not None, String("self is None"))
            self.value() = Float64(self.correct) / Float64(self.total)
fn update_from_batch(mut self, preds: List[Int], targets: List[Int]) -> None:
        var n = len(targets)
        var ok = 0
        for i in range(n):
            if preds[i] == targets[i]: ok += 1
...
struct WindowedAccuracy:
    ...
    var sum: Int        # sum(buf)
    var value: Float64
fn __init__(out self, window: Int) -> None:
...
fn _self_test() -> Bool:
    ...
    assert(meter is not None, String("meter is None"))
    ...
    assert(wmeter is not None, String("wmeter is None"))
