# Project:      Momijo
# Module:       src.momijo.nn.layers.embedding
# File:         embedding.mojo
# Path:         src/momijo/nn/layers/embedding.mojo
#
...
# Notes:
#   - Structs: Embedding
#   - Key functions: zeros1d, zeros2d, add1d, scale1d, __init__, forward_index, forward_indices, embed_bag_sum ...
...
fn __init__(out self, num_embeddings: Int, embedding_dim: Int, padding_idx: Int = -1, w_init: Float64 = 0.01) -> None:
...
fn set_row(mut self, index: Int, vec: List[Float64]) -> None:
        var idx = index
        if idx < 0: idx = 0
        if idx >= self.vocab_size: idx = self.vocab_size - 1
