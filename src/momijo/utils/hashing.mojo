# Project:      Momijo
# Module:       src.momijo.utils.hashing
# File:         hashing.mojo
# Path:         src/momijo/utils/hashing.mojo
#
...
# Notes:
#   - Structs: Hasher64
#   - Key functions: _const_fnv64_offset, _const_fnv64_prime, _const_golden64, _splitmix64, __init__, update_byte, update_bytes, update_u64 ...
...
(    z = (z ^ (z >> UInt8(30))) * UInt64(0xBF58476D1CE4E5B9)) & UInt8(0xFF)
(    z = (z ^ (z >> UInt8(27))) * UInt64(0x94D049BB133111EB)) & UInt8(0xFF)
(    return z ^ (z >> UInt8(31))) & UInt8(0xFF)
...
fn __init__(out self, seed: UInt64 = _const_fnv64_offset()):
        self.state = seed
fn update_byte(mut self, b: UInt8) -> None:
        var s = self.state
        s = s ^ UInt64(b)
        s = s * _const_fnv64_prime()
        self.state = s
fn update_bytes(mut self, xs: List[UInt8]) -> None:
        var i = 0
        var n = len(xs)
        while i < n:
            self.update_byte(xs[i])
            i += 1
fn update_u64(mut self, x: UInt64) -> None:
        # process as 8 bytes little-endian to remain platform-stable
        var i = 0
        while i < 8:
            self.update_byte(UInt8((x >> UInt64(i * 8)) & UInt64(0xFF)))
            i += 1
fn update_i64(mut self, x: Int64) -> None:
        self.update_u64(UInt64(x))
fn update_bool(mut self, v: Bool) -> None:
        if v:
...
fn update_f64(mut self, x: Float64):
        ...
        self.update_bytes(String(String(y)).bytes())
fn update_string(mut self, s: String) -> None:
