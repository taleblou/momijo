...
# ---------# -----------------------------------------------------------------------------
# DType: enum-like newtype for element data types
# -----------------------------------------------------------------------------
struct DType(Copyable, Movable):
    ...
    # Explicit copy helper (keeps parity with other project types)
    fn copy(self) -> DType:
    ...
    fn __ne__(self, other: DType) -> Bool: return self.id != other.id
    ...
    fn __str__(self) -> String:
        var out = String("DType(")
        out += String("id=") + String(self.id)
        out += String(", name=") + self.name()
        out += String(")")
        return out


# Convenience free helpers (kept for compatibility)
...
struct Layout(Copyable, Movable):
    ...
    fn __ne__(self, other: Layout) -> Bool: return self.id != other.id
    ...
    # Human-readable string
    fn __str__(self) -> String:
        var out = String("Layout(")
        out += String("id=") + String(self.id)
        out += String(", name=") + self.name()
        out += String(")")
        return out

# Free helpers for Layout (compat)
...
# -----------------------------------------------------------------------------
struct ColorSpace(Copyable, Movable):
    var id: Int
    ...
    # equality
    fn __eq__(self, other: ColorSpace) -> Bool: return self.id == other.id
    fn __ne__(self, other: ColorSpace) -> Bool: return self.id != other.id
    ...
    # Human-readable string
    fn __str__(self) -> String:
        var out = String("ColorSpace(")
        out += String("id=") + String(self.id)
        out += String(", name=") + self.name()
        out += String(")")
        return out

# Free helpers for ColorSpace (compat)
