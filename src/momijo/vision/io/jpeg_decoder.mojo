...
# Notes:
# - No globals (var-only), no ternary/match.
# - ExplicitlyCopyable & Movable where needed.
...
fn MARK_SOI() -> Int: return 0xFFD8
fn MARK_EOI() -> Int: return 0xFFD9
fn MARK_APP0() -> Int: return 0xFFE0
fn MARK_APP1() -> Int: return 0xFFE1
fn MARK_DQT()  -> Int: return 0xFFDB
fn MARK_SOF0() -> Int: return 0xFFC0
fn MARK_DHT()  -> Int: return 0xFFC4
fn MARK_SOS()  -> Int: return 0xFFDA
fn MARK_TEM()  -> Int: return 0xFF01
fn MARK_RST0() -> Int: return 0xFFD0  # ... up to RST7 (RST0 + 7)
...
struct JpegSegment(ExplicitlyCopyable, Movable):
    var marker: Int
...
struct ParseSegResult(ExplicitlyCopyable, Movable):
    var ok: Bool
    var seg: JpegSegment
    ...
    while (cursor + 2) <= end:
        ...
        if not has_length_field(marker):
            if marker == MARK_TEM():
                ...
                if is_rst(marker):
                    print("[JPEG] RST", marker - MARK_RST0())
                else:
                    print("[JPEG] Standalone marker 0x", marker)
            cursor = cursor + 2
            continue
...
fn decode_jpeg(ptr: UnsafePointer[UInt8], length: Int) -> (Bool, Image):
    ...
    while (cursor + 2) <= end:
        ...
        if marker == MARK_SOF0():
            ...
            var ok_dims, w, h, nf = parse_sof0_dims(res.seg.data_ptr, payload_len)
            if ok_dims:
                found_sof0 = True
                width = w
                height = h
    ...
    var npix = width * height * c
    var buf = UnsafePointer[UInt8].alloc(npix)
    var i = 0
    while i < npix:
        buf[i] = UInt8(0)   # black
        i = i + 1

    # Build tensor and image meta (sRGB)
    var t = Tensor.from_u8_hwc(buf, height, width, c)
    var meta = ImageMeta().with_colorspace(ColorSpace.SRGB())
    var img = Image(t, meta)
