...
#
# NOTE: Mojo checklist compliant — only 'var' (no 'let'), no 'export', no global mutable state,
# constructors use `fn __init__(out self, ...)`, prefer mut/out over inout, English comments,
# and printing only via String(...).
#
# Lightweight, dependency-free logging utilities with stable behavior.
# Focus: deterministic formatting, zero globals, easy to mock in tests.
...
struct Logger:
    ...
    fn set_level(mut self, lvl: UInt8):
        self.level = lvl
