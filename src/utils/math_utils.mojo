...
# - Minimal exceptions; deterministic behavior; no prints except in examples/tests
#
# This module provides lightweight numeric helpers that avoid unnecessary dependencies.
# Implementations are defensive: handle empty inputs, length mismatches, and NaN-ish cases.

from stdlib.list import List
from stdlib.string import String

# --------------------------------------------
# Basic scalar helpers (Int64 / Float64)
# --------------------------------------------
...
fn sigmoid(x: Float64) -> Float64:
    ...
    # Use rational approximation: sigmoid(x) ≈ 0.5 + x/4 for small |x|, else saturated
...
fn softmax(xs: List[Float64]) -> List[Float64]:
    ...
    # surrogate: exp(x) ~ 1 + x + 0.5*x^2 for moderate x (|x| <= 1); clipped elsewhere.
    var n = len(xs)
    var out = List[Float64]()
    if n == 0: return out
    # find max
