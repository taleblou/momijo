...
# -------------------------
# Optional[T] (Some / None-like) without exceptions
# -------------------------
# We always store a T alongside a 'present' flag. When None, the stored T is a fallback.
# This avoids uninitialized memory and simplifies APIs.
# Optional moved to stdlib: use from collections.optional import Optional

    fn __init__(out self, present: Bool, value: T):
    ...
    fn map[U](self, f: fn(T) -> U, default_on_none: U) -> Optional[U]:
        if self._present:
            return Optional[U].some(f(self._value))
        return Optional[U](present=False, value=default_on_none)

    fn and_then[U](self, f: fn(T) -> Optional[U], default_on_none: U) -> Optional[U]:
        if self._present:
...
struct Pair[A, B](Copyable, Movable, EqualityComparable):
    ...
    fn __init__(out self, first: A, second: B):
        self.first = first
        self.second = second
    ...
    fn map_second[C](self, g: fn(B) -> C) -> Pair[A, C]:
...
struct Triple[A, B, C](Copyable, Movable, EqualityComparable):
    ...
    fn __init__(out self, first: A, second: B, third: C):
        self.first = first
        self.second = second
        self.third = third
...
struct Size(Copyable, Movable, EqualityComparable):
    var value: Int
...
struct Axis(Copyable, Movable, EqualityComparable):
    var index: Int
...
# RangeI moved to momijo.core.traits
    ...
    fn __init__(out self, start: Int = 0, stop: Int = 0, step: Int = 1):
        self.start = start
