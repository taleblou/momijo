...
# -------------------------
# Core description / printing / hashing
# -------------------------
...
struct SeqList[T](Copyable, Movable, EqualityComparable):
    var _data: List[T]

    fn __init__(out self, data: List[T] = List[T]()):
        self._data = data
...
struct ListIterator[T](Copyable, Movable, EqualityComparable):
    var _data: List[T]
    var _idx: Int
...
struct TensorMeta(Copyable, Movable, EqualityComparable):
    var _shape: Shape
    var _dtype: DType
    var _device: String

    fn __init__(out self, shape: Shape = Shape(dims=[]), dtype: DType = DType.f32(), device: String = "cpu"):
        self._shape = shape
        self._dtype = dtype
        self._device = device
...
@staticmethod
fn order_lt[T: Orderable](a: T, b: T) -> Bool: return a.compare(b) < 0
@staticmethod
fn order_le[T: Orderable](a: T, b: T) -> Bool: return a.compare(b) <= 0
@staticmethod
fn order_gt[T: Orderable](a: T, b: T) -> Bool: return a.compare(b) > 0
@staticmethod
fn order_ge[T: Orderable](a: T, b: T) -> Bool: return a.compare(b) >= 0

@staticmethod
fn numeric_add[T: Numeric[T]](x: T, y: T) -> T: return x.add(y)
@staticmethod
fn numeric_sub[T: Numeric[T]](x: T, y: T) -> T: return x.sub(y)
@staticmethod
fn numeric_mul[T: Numeric[T]](x: T, y: T) -> T: return x.mul(y)
@staticmethod
fn numeric_div[T: Numeric[T]](x: T, y: T) -> T: return x.div(y)