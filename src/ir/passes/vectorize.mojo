...
#
# Project: momijo.ir
# File: src/momijo/ir/vectorize.mojo
# Description: Vectorization annotator/splitter for Momijo IR elementwise ops.
#              Targets 'vec.add/sub/mul/div.{i64|f64}' and 'vec.fused.{i64|f64}'.
#              Adds a 'vector=<W>' attribute (default W=4 when legal) and, if N
#              is not a multiple of W, splits the op into a main (vectorized)
#              part and a scalar tail with 'offset=<start>'.
# Notes:
#   - Self-contained fallback IR; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Assumes ops have attrs like: {N=..., [unroll=...], ...}. We do not change
#     semantics besides splitting/annotating.
#   - Emits an informational note: 'vectorize.note {split=true|false, width=W}'.
...
struct Vectorize:
    ...
    fn _choose_width(self, dtype: String, N: Int) -> Int:
        # Simple heuristic: i64/f64 → try 4-way; otherwise 1
...
fn _demo_module() -> Module:
    ...
    # Case 1: vec.add.i64 {N=16} → vector=4, no split
    ...
    # Case 2: vec.fused.i64 {N=18} → split: (N=16,vector=4) + (N=2,vector=1,offset=16)
