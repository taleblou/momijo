...
#
# Project: momijo.ir
# File: src/momijo/ir/fusion_elemwise.mojo
# Description: Elementwise fusion pass. Fuses short chains of vector (1-D) ops
#              like vec.add/vec.mul with matching shape into a single vec.fused.*
#              op carrying an RPN expression. Intended to run before loop_nest.
# Notes:
#   - Self-contained fallback IR; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Recognized inputs (buffers 1-D):
#       vec.add.{i64,f64}(out, a, b) {N=...[, unroll=?]}
#       vec.sub.{i64,f64}(out, a, b) {N=...[, unroll=?]}
#       vec.mul.{i64,f64}(out, a, b) {N=...[, unroll=?]}
#       vec.div.{i64,f64}(out, a, b) {N=...[, unroll=?]}
#   - Pattern fused (two-stage):
#       tmp = vec.OP1(a,b); out = vec.OP2(tmp,c)    or    out = vec.OP2(c,tmp)
#       → vec.fused.{i64,f64}(out, a, b, c) {N=..., [unroll], arity=3, expr_rpn="x0 x1 OP1 x2 OP2"}
#     where OP ∈ {add,sub,mul,div}. Temp 'tmp' may be an allocated buffer used once.
#   - Emits "fusion.removed {from=...}" note for the producer op.
#   - Leaves any now-unused buf.alloc to later DCE.
#   - Includes tiny printer & self-test that must print OK.
# ============================================================================
