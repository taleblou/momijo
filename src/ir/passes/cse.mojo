...
#
# Project: momijo.ir
# File: src/momijo/ir/cse.mojo
# Description: Common Subexpression Elimination (CSE) pass for Momijo IR
#              (per-block, SSA). Handles commutativity and preserves effectful ops.
# Notes:
#   - Self-contained fallback IR; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Keys by (opname, normalized attrs, normalized operand value-ids).
#   - Commutativity: add/mul/and/or/cmp.eq treated as order-independent.
#   - Effectful ops (stores, returns, loops, rt.*, calls) are NOT eliminated.
#   - Rewrites subsequent operands to alias representatives as it goes.
#   - Emits "cse.removed {from=..., alias=%id}" notes when removing an op.
...
struct AliasMap:
    ...
    fn __init__(out self): self.parent = Dict[Int, Int]()
