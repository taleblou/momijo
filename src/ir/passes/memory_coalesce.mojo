...
#
# Project: momijo.ir
# File: src/momijo/ir/memory_coalesce.mojo
# Description: Memory coalescing / copy-loop elimination for Momijo IR.
#              Detects trivial elementwise copy loops and replaces them with
#              a single contiguous 'buf.memcpy' op, optionally annotated with
#              a vector width ("coalesce.width") when length is divisible by 4.
# Notes:
#   - Self-contained fallback IR; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Recognizes the loop_nest-style pattern:
...
#          buf.store %x, dst, %iv
#        loop.end
#     → buf.memcpy(dst, src) {len=N, elem=<dtype>, coalesce=true[, coalesce.width=4]}
...
struct ShapeInfo:
    var dtype: String
    fn __init__(out self): self.dtype = String("")
