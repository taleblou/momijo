...
#
# Project: momijo.ir
# File: src/momijo/ir/algebraic_simplify.mojo
# Description: Algebraic simplification / constant folding for Momijo IR.
# Notes:
#   - Self-contained fallback IR; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Works across dialect prefixes: hl.*, mid.*, ll.* where applicable.
#   - Rules (subset):
#       * Const folding: add/sub/mul/div on {i64} (safe div by nonzero)
#       * Identities:   x+0 -> x, 0+x -> x, x-0 -> x, x*1 -> x, 1*x -> x,
#                       x*0 -> 0, 0*x -> 0, x/1 -> x, 0/x -> 0
#       * Cmp:          eq(x,x)->true, lt(x,x)->false, gt(x,x)->false, fold if both consts
#       * Select:       select(true,t,f)->t, select(false,t,f)->f, select(c,x,x)->x
#   - Implementation maintains a value-alias & constant map during a single forward pass.
#   - Emits "simp.removed {from=...}" note ops for replaced instructions.
...
struct ConstInfo:
    ...
    fn __init__(out self): self.kind = ConstKind.None_; self.s = String("")
...
struct AlgebraicSimplify:
    ...
    fn _try_simplify_cmp(self, b: Block, idx: Int) -> Bool:
        ...
        # mid.cmp with pred attr → normalize to eq/lt/gt
        var name = op.name
        if _is_mid_cmp(name):
