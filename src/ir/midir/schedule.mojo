...
#
# Project: momijo.ir
# File: src/momijo/ir/schedule.mojo
# Description: Operation scheduler for Momijo IR (per-block). Provides:
#   - DAG build from SSA deps
#   - ASAP times (with per-op latency)
#   - Resource‑constrained list scheduling (per-cycle capacities)
#   - Reorders ops by scheduled cycle; annotates with {cycle=, latency=, res=?}
# Notes:
...
#       * op attr "latency=<int>" overrides
#       * otherwise latency from options.op_latency_by_prefix (by starts_with key)
#       * otherwise default_latency
#   - Resource binding:
#       * op attr "res=<name>" sets a resource name
#       * otherwise bound by options.resource_by_prefix (by starts_with key)
...
struct NodeInfo:
    ...
    var latency: Int
    var resource: String
    fn __init__(out self, op_index: Int):
