...
#
# Project: momijo.ir
# File: src/momijo/ir/loop_nest.mojo
# Description: Loop-nest builder and a lowering pass that converts simple
#              vector (elementwise) buffer ops (vec.*) into explicit loop IR.
# Notes:
#   - Self-contained fallback IR model; no globals, no 'export'; only 'var' (no 'let').
#   - Constructors use: fn __init__(out self, ...)
#   - Recognized patterns (1-D, buffers):
#       vec.add.{i64,f64}(out:buffer, a:buffer, b:buffer) {N=...[, unroll=?]}
#       vec.mul.{i64,f64}(out:buffer, a:buffer, b:buffer) {N=...[, unroll=?]}
#   - Lowering result (schematic):
#       %c0 = ll.const.i64 value=0
#       %cN = ll.const.i64 value=N
#       %c1 = ll.const.i64 value=1
#       %iv = loop.for(%c0,%cN,%c1) -> i64
#         %x  = buf.load a, %iv
#         %y  = buf.load b, %iv
#         %z  = ll.add %x, %y
#         buf.store %z, out, %iv
#       loop.end
#   - Optional unrolling (integer 'unroll' attr) replicates the body with iv+u.
#     Assumes N % unroll == 0 (minimal implementation).
#   - Provides a tiny printer and a self-test that must print OK.
# ============================================================================
...
struct Builders:
    var idg: IdGen
    fn __init__(out self): self.idg = IdGen()
...
# -----------------------------
# Lowerer: vec.* → loop nest with buf.load/ll.add/ll.mul/buf.store
# -----------------------------
