...
#
# Project: momijo.ir
# File: src/momijo/ir/midir_to_llir.mojo
# Description: Lowering pass from MIDIR (portable mid-level IR) to LLIR
#              (low-level, closer-to-target IR).
# Notes:
#   - Self-contained fallback IR model; no globals, no 'export'; only 'var' (no 'let').
#   - Clones function signatures and blocks; renames ops 'mid.*' → 'll.*' with small tweaks.
#   - Keeps TypeDesc names intact, preserves attributes (e.g., constants' value and cmp's pred).
...
# mid.cmp {pred=EQ|LT|GT} → ll.cmp.<pred>
