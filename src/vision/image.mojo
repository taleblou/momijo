...
#
# A minimal, dependency-light Image container for Momijo Vision.
# Design goals:
#   - Standalone (no imports from other WIP modules)
#   - Safe defaults, explicit sizing, simple pixel helpers
#   - U8/HWC storage with channel-order + alpha metadata
# Style rules:
#   - No 'export', no 'let', no 'inout'
#   - Constructors use `fn __init__(out self, ...)`
#   - Mutations are via free functions returning new values
#
# NOTE: When dtypes.mojo is stabilized, we can replace the local tags below
#       with imports from momijo.vision.dtypes.

# -------------------------
# Local tag types (ChannelOrder / Layout / AlphaMode)
# -------------------------
