...
#
# Minimal PPM (P6/P3) utilities for Momijo Vision (standalone, dependency-light).
# Goals:
#   - Validate PPM magic (P6 = binary RGB, P3 = ASCII RGB)
#   - Parse metadata: width, height, maxval, magic, data_offset
#   - Decode to packed RGB UInt8 image (scaling if maxval != 255)
#   - Encode from packed RGB to P6 (binary) with maxval=255
# Style:
#   - No 'export', no 'let', no 'inout'
#   - Constructors use `fn __init__(out self, ...)`
#
# NOTE: Not a full PPM implementation; focuses on common cases and safety.
#       P6: supports maxval in [1,255]; sample size must be 1 byte. P3: parses ASCII integers.

# -------------------------
# Lightweight RGB Image (local to avoid imports)
# -------------------------
struct ImageRGBU8(Copyable, Movable):
    ...
    fn __init__(out self, h: Int, w: Int, data: List[UInt8]):
        self.h = h; self.w = w
        var expected = h * w * 3
