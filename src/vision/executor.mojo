...
#
# A minimal, dependency-light vision executor that can run a small pipeline of ops
# on a uint8 HWC image. Designed to compile without importing other Momijo modules.
# Style rules: no export, no let, no inout. Constructors use `fn __init__(out self, ...)`.
# Mutating behavior is implemented via free functions returning new values.

# -------------------------
# Lightweight helpers (Channel count & layout tags kept local to avoid imports)
# -------------------------
...
fn rgb_to_gray(src: ImageU8HWC) -> ImageU8HWC:
    ...
    while y < src.h:
        ...
        while x < src.w:
            ...
            var gray = UInt8((gray_u16 >> 8) & UInt16(0xFF))
