...
#
# High-level resize helpers for Momijo Vision.
# Standalone & dependency-light: no imports; all helpers are defined here.
# Style: no 'export', no 'let', no 'inout'. Constructors use `fn __init__(out self, ...)`.
#
# Implemented:
#   - resize_nearest_u8_hwc(h,w,c, src, oh,ow) -> List[UInt8]
#   - resize_bilinear_u8_hwc(h,w,c, src, oh,ow) -> List[UInt8]  (fixed-point 8.8)
#   - Image wrappers: apply_resize_* (preserve channel order), and a generic apply_resize with Interp
#
# Notes:
# - src/out are flat UInt8 buffers in HWC layout.
# - Bilinear handles degenerate sizes (ow==1/oh==1).
#
# -------------------------
