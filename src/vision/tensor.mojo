...
#
# Minimal Vision Tensor (uint8) with HWC/CHW support and explicit strides.
# Standalone by design: no imports from other Momijo modules.
# Style: no 'export', no 'let', no 'inout'. Use `fn __init__(out self, ...)`.
#
# WARNING: This implementation focuses on U8 image tensors. DType tag is present
# for forward-compatibility but only UInt8 is actually backed by storage here.
...
struct Tensor(Copyable, Movable):
    ...
    fn __init__(out self, h: Int, w: Int, c: Int, layout: Layout, dtype: DType, s0: Int, s1: Int, s2: Int, data: List[UInt8]):
        self.h = h
        self.w = w
        self.c = c
        self.layout = layout
