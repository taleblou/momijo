...
# File: src/momijo/vision/registry.mojo
#
# A minimal, dependency-light operation registry for Momijo Vision.
# Purpose:
#   - Provide a central list of supported ops (name, kind, param count, doc)
#   - Resolve user-facing strings to OpKind ids
#   - Offer a lightweight Node builder (kind + up to 2 integer params)
# Notes:
#   - Standalone by design (no imports from other Momijo modules)
#   - Style: no 'export', no 'let', no 'inout'; constructors as `fn __init__(out self, ...)`
#
# When Executor/Image/Tensor APIs are stabilized, this registry can be wired to them.
...
struct OpSpec(Copyable, Movable):
    ...
    fn __init__(out self, kind: OpKind, name: String, num_params: Int, impl: ImplState, doc: String):
        self.kind = kind
        self.name = name
...
struct Node(Copyable, Movable):
    ...
    fn __init__(out self, kind: OpKind, p1: Int, p2: Int):
        self.kind = kind
        self.p1 = p1
        self.p2 = p2
