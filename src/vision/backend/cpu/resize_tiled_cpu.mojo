...
# File: src/momijo/vision/backend/cpu/resize_tiled_cpu.mojo
#
# CPU tiled resize kernels for uint8 images in HWC layout.
# Standalone & dependency-light (no external Momijo imports).
# Style: no 'export', no 'let', no 'inout'.
#
# Implemented ops (generic over channel count c), using cache-friendly tiles:
#   - resize_nearest_tiled_u8_hwc(h,w,c, src, oh,ow, tile_h, tile_w) -> List[UInt8]
#   - resize_bilinear_tiled_u8_hwc(h,w,c, src, oh,ow, tile_h, tile_w) -> List[UInt8]
#
# Notes:
# - src is a flat List[UInt8] of length h*w*c (row-major HWC).
# - Output is a new List[UInt8] of length oh*ow*c.
# - Tiling only affects loop order (for locality); results match non-tiled versions.
