...
# File: src/momijo/vision/backend/cpu/simd/resize_simd_u8_hwc.mojo
#
# "SIMD-friendly" (auto-vectorizable) resize kernels for uint8 images in HWC layout.
# No external SIMD types are used to keep portability; loops are chunked/unrolled
# to help the compiler auto-vectorize.
# Style: no 'export', no 'let', no 'inout'.
#
# Implemented ops (generic over channel count c):
#   - simd_resize_nearest_u8_hwc(h, w, c, src, oh, ow) -> List[UInt8]
#   - simd_resize_bilinear_u8_hwc(h, w, c, src, oh, ow) -> List[UInt8] (fixed-point 8.8)
#
# Notes:
# - src is a flat List[UInt8] of length h*w*c in row-major HWC.
# - Output is a new List[UInt8] of length oh*ow*c.
# - Bilinear uses fixed-point weights: sx_fp = (x*(w-1)*256)/(ow-1), similarly for y.
#   Handles degenerate cases (ow==1 or oh==1).
...
fn simd_resize_bilinear_u8_hwc(h: Int, w: Int, c: Int, src: List[UInt8], oh: Int, ow: Int) -> List[UInt8]:
    ...
    while y < oh:
        ...
        while x + 3 < ow:
            ...
            var x0_0 = sx0_fp >> 8; var fx0 = sx0_fp & 255; var x1_0 = x0_0 + 1; if x1_0 >= w: x1_0 = w - 1
            var x0_1 = sx1_fp >> 8; var fx1 = sx1_fp & 255; var x1_1 = x0_1 + 1; if x1_1 >= w: x1_1 = w - 1
            var x0_2 = sx2_fp >> 8; var fx2 = sx2_fp & 255; var x1_2 = x0_2 + 1; if x1_2 >= w: x1_2 = w - 1
            var x0_3 = sx3_fp >> 8; var fx3 = sx3_fp & 255; var x1_3 = x0_3 + 1; if x1_3 >= w: x1_3 = w - 1
