...
# File: src/momijo/vision/ir.mojo
#
# Minimal, dependency-light Intermediate Representation (IR) for Momijo Vision.
# Goals:
#   - Define OpKind, Node, Param, Graph, and ExecStep as a stable core.
#   - Offer helpers to build graphs, set params, validate, topologically sort,
#     and compile to a linear execution plan (ExecStep list).
# Style:
#   - No 'export', no 'let', no 'inout'.
#   - Constructors use `fn __init__(out self, ...)`.
# Notes:
#   - This file is self-contained and does not import other Momijo modules.
#   - OpKind names align with momijo.vision.registry for easy wiring later.

# -------------------------
# OpKind tag (keep in sync with vision.registry)
# -------------------------
...
struct Node(Copyable, Movable):
    ...
    fn __init__(out self, id: Int, kind: OpKind, name: String):
        self.id = id
        self.kind = kind
        self.name = name
...
struct Edge(Copyable, Movable):
    var src: Int
    var dst: Int
    fn __init__(out self, src: Int, dst: Int):
        self.src = src
        self.dst = dst
...
struct ExecStep(Copyable, Movable):
    ...
    var p1: Int
    var p2: Int
    fn __init__(out self, node_id: Int, kind: OpKind, p1: Int, p2: Int):
