...
#
# Project: momijo.vision.fusion
# File: src/momijo/vision/fusion.mojo
#
# Minimal, dependency-light image fusion/compositing utilities for Momijo Vision.
# Focused on UInt8 images in HWC layout, generic over channel count (c ∈ {1,3,4}).
# Style: no 'export', no 'let', no 'inout'; constructors use `fn __init__(out self, ...)`.
#
# Implemented:
#   - blend_same_size_u8_hwc(mode, h,w,c, A, B, opacity)  -> out (A ⊗ B)
#   - alpha_over_u8_hwc(bg_h,bg_w,c, BG, fg_h,fg_w,fg_c, FG, x,y, opacity) -> composited BG
#     (FG may be RGB or RGBA. If RGBA, uses per-pixel alpha; otherwise uses scalar opacity)
#
# Notes:
# - All outputs allocate a fresh buffer to avoid aliasing.
# - Alpha math uses integer fixed-point (0..255).
...
# out = (1 - t) * A  ⊗  t * B  with mode ⊗ and scalar opacity t in [0..255]
# -------------------------
@staticmethod
