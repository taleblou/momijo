...
# File: src/momijo/vision/random_horizontal_flip.mojo
#
# Minimal, dependency-light Random Horizontal Flip (HFlip) for Momijo Vision.
# Focus: UInt8 images in HWC layout (c ∈ {1,3,4}).
# Style: no 'export', no 'let', no 'inout'. Constructors use `fn __init__(out self, ...)`.
#
# Implemented:
#   - flip_h_u8_hwc(h,w,c, src) -> List[UInt8]                        # deterministic flip
#   - maybe_flip_h_u8_hwc(h,w,c, src, do_flip: Bool) -> List[UInt8]
#   - LcgRng (simple RNG) + bernoulli_q8(p_q8)                         # p_q8 in [0..255] ~ [0.0..0.996]
#   - random_flip_h_u8_hwc(h,w,c, src, seed, p_q8) -> (List[UInt8], Bool)
#   - Image wrapper: apply_random_hflip(img, seed, p_q8) -> (Image, Bool)
#   - Batch helpers: sample_masks(n, seed, p_q8) -> List[Bool], apply_random_hflip_batch(...)
#   - __self_test__()
#
# Notes:
# - Probability is represented in Q8 scale: 0 => 0%, 255 => ~100% (accept if rand in [0..254] < p_q8).
# - RNG is a simple 31-bit LCG; deterministic across runs for the same seed.
# - All functions allocate fresh output buffers to avoid aliasing.
