...
# File: src/momijo/vision/random_rotation.mojo
#
# Minimal, dependency-light Random Rotation for Momijo Vision.
# Focus: UInt8 images in HWC layout (c ∈ {1,3,4}).
# Style: no 'export', no 'let', no 'inout'. Constructors use `fn __init__(out self, ...)`.
#
# Implemented:
#   - rotate_bilinear_u8_hwc_keep(h,w,c, src, deg, pad) -> List[UInt8]   # same output size as input
#   - rotate_bilinear_u8_hwc_auto(h,w,c, src, deg, pad) -> (Int, Int, List[UInt8]) # auto-sized canvas
#   - LcgRng RNG + uniform_int for picking degrees in [min_deg..max_deg]
#   - apply_random_rotation(img, seed, min_deg, max_deg, pad_mode, keep_size) -> (Image, Int) 
#     * keep_size: 1 => keep input size, 0 => auto-size
#   - __self_test__()
#
# Notes:
# - Uses fixed-point Q16 for sin/cos; inverse mapping with bilinear sampling.
# - PadMode: ZERO (0) or EDGE (1). EDGE clamps source coords when sampling.
# - For deg=0, it fast-paths to a copy.
#
# -------------------------
# Common image types
# -------------------------
...
fn _sample_bilinear_u8_hwc(h: Int, w: Int, c: Int, src: List[UInt8], fx_q16: Int, fy_q16: Int, pad: PadMode) -> List[UInt8]:
    ...
    var fx = fx_q16 & 0xFFFF
    var fy = fy_q16 & 0xFFFF
    var wx0 = 65536 - fx
    var wy0 = 65536 - fy
...
fn rotate_bilinear_u8_hwc_keep(h: Int, w: Int, c: Int, src: List[UInt8], deg: Int, pad: PadMode) -> List[UInt8]:
    ...
    var cx_q16 = ((w << 16) - 65536) >> 1  # (w-1)/2 in Q16
    var cy_q16 = ((h << 16) - 65536) >> 1

    var out = _alloc_u8(h*w*c)
    var y = 0
    while y < h:
        var dy_q16 = (y << 16) - cy_q16
        var x = 0
        while x < w:
            var dx_q16 = (x << 16) - cx_q16
            # inverse map
...
fn rotate_bilinear_u8_hwc_auto(h: Int, w: Int, c: Int, src: List[UInt8], deg: Int, pad: PadMode) -> (Int, Int, List[UInt8]):
    ...
    var cx_src_q16 = ((w << 16) - 65536) >> 1
    var cy_src_q16 = ((h << 16) - 65536) >> 1
    var cx_dst_q16 = ((ow << 16) - 65536) >> 1
    var cy_dst_q16 = ((oh << 16) - 65536) >> 1

    var out = _alloc_u8(oh*ow*c)
    var y = 0
    while y < oh:
        var dy_q16 = (y << 16) - cy_dst_q16
        var x = 0
        while x < ow:
            var dx_q16 = (x << 16) - cx_dst_q16
            # map dest->src
