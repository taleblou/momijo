...
# File: src/momijo/vision/normalize.mojo
#
# Minimal, dependency-light normalization utilities for Momijo Vision.
# Focused on HWC layout images. Supports UInt8 I/O and Float32 pipelines.
# Style: no 'export', no 'let', no 'inout'. Constructors use `fn __init__(out self, ...)`.
#
# Implemented:
#   - Compute per-channel min/max for UInt8 buffers
#   - Min-Max normalization (global or per-channel) to UInt8 range
#   - Convert UInt8 → Float32 unit scale [0,1] and back
#   - Mean/Std normalization (per-channel) on Float32
#   - Lightweight ImageU8/ImageF32 containers + wrappers
#   - __self_test__() smoke
#
# Notes:
# - Per-channel arrays are expected to have length == c.
# - Float32 path is preferred for model input (e.g., mean/std normalization).
# - UInt8 path is handy for visualization or classical CV stages.

# -------------------------
# Lightweight channel/order & image structs
# -------------------------
...
struct ImageF32(Copyable, Movable):
    ...
    fn __init__(out self, h: Int, w: Int, order: ChannelOrder, data: List[Float32]):
        self.h = h; self.w = w; self.order = order
        var expected = h*w*_num_ch(order)
        if len(data) != expected:
