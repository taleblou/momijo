...
# File: src/momijo/vision/compose.mojo
#
# Minimal, dependency-light transform composition for Momijo Vision.
# Provides a simple pipeline of image transforms operating on UInt8/HWC buffers
# via a lightweight Image container defined locally (no imports).
# Style:
...
# Implemented:
#   - Image + ChannelOrder
#   - Enums: Interp, PadMode, TKind (transform kinds)
#   - Core kernels used by transforms: resize (nearest/bilinear), center_crop, bgr_to_rgb, rgb_to_gray, drop_alpha,
#     color jitter (brightness/contrast/saturation/hue)
#   - Transform (kind + params) and Pipeline (list of transforms)
#   - Validation, summary, and run(img) -> img
#   - __self_test__()
#
# Notes:
# - This module is intentionally self-contained to make early integration easy.
# - When wiring with backend modules, these impls can be swapped with calls to
#   the optimized backends keeping the same signatures.
...
fn adjust_hue_u8_hwc(h: Int, w: Int, c: Int, src: List[UInt8], deg: Int) -> List[UInt8]:
    ...
    while i < h*w:
        ...
        var r14 = Y + (RY * I2 >> 14) + (RQ * Q2 >> 14)
        var g14 = (GY * (Y >> 14) << 14) + (GI * I2 >> 14) + (GQ * Q2 >> 14)
        var b14 = (BY * (Y >> 14) << 14) + (BI * I2 >> 14) + (BQ * Q2 >> 14)
        var rr = r14 >> 14; var gg = g14 >> 14; var bb = b14 >> 14
        if c == 1:
...
struct Transform(Copyable, Movable):
    ...
    var p3: Int
    var p4: Int
    fn __init__(out self, kind: TKind):
