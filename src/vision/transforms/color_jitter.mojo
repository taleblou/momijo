...
# File: src/momijo/vision/color_jitter.mojo
#
# Minimal, dependency-light Color Jitter for Momijo Vision.
# Focused on UInt8 images in HWC layout (c ∈ {1,3,4}).
# Implemented ops (all channel-safe, alpha preserved if c==4):
#   - adjust_brightness_u8_hwc(h,w,c, src, delta)                 # delta ∈ [-255..255]
#   - adjust_contrast_u8_hwc(h,w,c, src, factor_q8)               # factor in Q8.8 (256 == 1.0)
#   - adjust_saturation_u8_hwc(h,w,c, src, factor_q8)             # factor in Q8.8
#   - adjust_hue_u8_hwc(h,w,c, src, deg)                          # deg ∈ [-180..180], YIQ rotation (fixed-point)
#   - apply_color_jitter_u8_hwc(..., delta, contrast_q8, sat_q8, deg)  # combined
#   - LCG RNG + sample_jitter(...) for random params
#   - Lightweight Image wrappers
#
...
fn _sin_q16(x_q16: Int) -> Int:
    # Fast sine approximation on [-pi,pi]: s ≈ A*x - B*x*|x|, Q16 in/Q16 out.
    # A ≈ 1.27323954, B ≈ 0.405284735
...
fn adjust_hue_u8_hwc(h: Int, w: Int, c: Int, src: List[UInt8], deg: Int) -> List[UInt8]:
    ...
    while i < h * w:
        ...
        var g14 = (GY * (Y >> 14) << 14) + (GI * I2 >> 14) + (GQ * Q2 >> 14)  # keep Y scale
...
struct JitterParams(Copyable, Movable):
    ...
    var hue_deg: Int
    fn __init__(out self, bd: Int, cq8: Int, sq8: Int, hd: Int):
