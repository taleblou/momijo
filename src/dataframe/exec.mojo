...
# File: momijo/dataframe/exec.mojo

from momijo.dataframe.logical_plan import LogicalPlan, PLAN_SCAN, PLAN_FILTER, PLAN_PROJECT, PLAN_AGGREGATE, PLAN_SORT, PLAN_JOIN, PLAN_WINDOW, AggSpec
...
from momijo.dataframe.expr import eval_expr
from momijo.dataframe.join import join_inner, join_left, join_right, join_full, join_anti
from momijo.dataframe.window import rolling_mean, row_number, lag, lead, cum_sum, cum_mean, dense_rank

# -----------------------------------------------------------------------------
# Executor
# -----------------------------------------------------------------------------
# Executes a LogicalPlan into a concrete DataFrame. The executor is purely
# functional (no mutation on input DataFrames), so it composes cleanly.
...
fn groupby_multi(df: DataFrame, keys: List[String], aggs: List[AggSpec]) -> DataFrame:
    ...
    # Composite keys for each row: "k1␟k2␟...␟"
        ...
        while g < G:
            ...
            while r < n:
                if gids[r] == g:
                    ...
                    found = df.cols[cidx].value_str(r)
                    break
                r += 1
