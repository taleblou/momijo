...
# File: momijo/dataframe/frame.mojo
...
struct DataFrame(Copyable, Movable):
    ...
    fn __init__(out self, names: List[String], cols: List[Column]):
        self.names = List[String]()
        self.cols = List[Column]()
        ...
        fn less(i: Int, j: Int) -> Bool:
            ...
            while k < len(keys):
                ...
                if c.tag == ColumnTag.F64() or c.tag == ColumnTag.I64():
                    ...
                    var a_s = c.value_str(i)
                    var b_s = c.value_str(j)
                    if a_s < b_s: res = -1
                    elif a_s > b_s: res = 1
                if res != 0:
